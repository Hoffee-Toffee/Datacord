    ...
    (bot, message) => {
      if (message.user.username !== realbotname) {
        ...
        if (txt.toLowerCase() == (prefix + "line").toLowerCase()) {
          var line = linesdata.lines[Math.round(Math.floor(Math.random() * linesdata.lines.length))]
...
controller.hears(
    ...
    (bot, message) => {
      if (message.user.username !== realbotname) {
        ...
        if (choice == 0) {
          bot.reply(message, greeting[Math.round(Math.floor(Math.random() * 2))] + word + ", I'm " + botname + "!");
        }
        else if (choice == 1) {
          bot.reply(message, greeting[Math.round(Math.floor(Math.random() * 2))] + word + ", I thought you were " + message.user.username + "?");
        }
        else {
          bot.reply(message, "But I thought you were " + message.user.username + "?");
        }
      }
    }
  );
  ...
  controller.hears(
    ...
    ["ambient", "direct_message", "mention"],
    (bot, message) => {
      if (message.user.username !== realbotname && Math.round(Math.floor(Math.random() * 10)) == 1) {
